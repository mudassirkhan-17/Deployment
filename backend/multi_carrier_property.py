...
from A_phase3_llm_extraction import read_combined_file, create_chunks, extract_with_llm, merge_extraction_results
...
def save_carrier_extraction_results(carrier_name, merged_result, all_results):
    """Save carrier-specific extraction results - SIMPLIFIED VERSION (ONE FILE ONLY)"""
    # DYNAMIC PATH DETECTION
    current_dir = os.getcwd()
    if current_dir.endswith('src'):
        results_dir = '../results'
    ...
    # ONLY FILE: final_validated_fields.json (consumed by Google Sheets)
    final_file = f"{results_dir}/{carrier_name}_final_validated_fields.json"
    
    # Get page sources from metadata
    ...
    for field_name, field_value in merged_result.items():
        ...
        source_page = f"Page {source_pages[0]}" if source_pages else ""
...
def create_carrier_final_validated_fields(carrier_name, merged_result):
    """DEPRECATED - Now handled in save_carrier_extraction_results"""
    pass
...
def rename_files_with_carrier_prefix(carrier_name, phase_type):
    ...
    # List of files to rename based on phase (UPDATED for simplified phases)
    if phase_type == "phase1":
        files_to_rename = [
            'phase1_report.txt',
            'pymupdf_extracted_text.txt'
        ]
