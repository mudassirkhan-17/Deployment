...
class PolicyPageExtractor:
    ...
    def find_headings(self) -> Dict[str, List[HeadingMatch]]:
        """
        OPTION 2: Find ALL declaration pages (not coverage-specific).
        Extract all pages with DECLARATIONS keyword + financial data.
        No coverage separation - mix everything together.
        Let LLM handle coverage separation.
        """
        
        # Find ALL pages with "DECLARATIONS" keyword that have actual policy data
        all_declaration_pages = []
        
        matches_found = set()  # Track positions to avoid duplicates
        
        # Search for declaration pages - simple, powerful pattern
        for match in re.finditer(r'DECLARATIONS', self.policy_text, re.IGNORECASE):
            char_pos = match.start()
            
            # Skip if we already found a declaration near this position
            is_duplicate = False
            for found_pos in matches_found:
                if abs(char_pos - found_pos) < 500:  # If within 500 chars, same declaration
                    is_duplicate = True
                    break
            ...
            if not is_duplicate:
                ...
                # Skip if page has "EXAMPLE" or "CALCULATION" (indicates it's instructional, not declarations)
                if 'EXAMPLE' in page_text.upper() or 'CALCULATION' in page_text.upper():
                    continue
                
                # Check if page has dollar amounts (real coverage limits, not just text)
                has_amounts = bool(re.search(r'\$\s*[0-9,]+', page_text))
                
                if has_amounts:
                    # This is a real declaration page
                    all_declaration_pages.append({
                        'page': page_num,
                        'char_pos': char_pos,
                        'line_num': line_num
                    })
                    matches_found.add(char_pos)
        
        # Sort by page number (earlier pages first)
        all_declaration_pages.sort(key=lambda x: x['page'])
        
        # NEW: Use ALL declaration pages found (up to 10)
        # Extract pages around EACH declaration page
        # This ensures we get coverage-specific declarations like "GL COVERAGE PART DECLARATIONS" on page 18
        # The LLM will receive mixed content and use neighboring context to sort by coverage
        
        # Limit to top 10 declaration pages found
        top_declaration_pages = all_declaration_pages[:10]
        
        print(f"[DEBUG] Found {len(all_declaration_pages)} total declaration pages")
        print(f"[DEBUG] Using top {len(top_declaration_pages)} for extraction")
        ...
        # For each coverage type, map ALL declaration pages found
        # This way each coverage gets multiple page ranges, increasing chance of finding coverage-specific declarations
        if top_declaration_pages:
            for coverage in ['GL', 'PROPERTY', 'EPLI', 'LIQUOR', 'WC']:
                # Create matches for all declaration pages found
                for i, decl_page in enumerate(top_declaration_pages):
                    heading_match = HeadingMatch(
                        coverage=coverage,
                        pattern=f'DECLARATIONS (Match {i+1}/{len(top_declaration_pages)})',
                        match_text='DECLARATIONS',
                        char_position=decl_page['char_pos'],
                        line_number=decl_page['line_num'],
                        page_number=decl_page['page']
                    )
                    all_matches[coverage].append(heading_match)
                
                self.extraction_log['headings_found'][coverage] = len(top_declaration_pages)
    ...
    def process_all_headings(self) -> Dict:
        ...
        # Extract sections
        print(f"\n[EXTRACTING SECTIONS - 5 pages each]:\n")
        ...
        for coverage, matches in headings.items():
            ...
            print(f"  {coverage}:")
            print(f"    Found {len(matches)} declaration page(s): ", end="")
            
            # Extract pages around EACH match and combine them
            # This ensures we get coverage-specific declarations from different pages
            combined_text = ""
            page_ranges = []
    ...
    def validate_extractions(self, sections: Dict) -> Dict:
        ...
        for coverage, section in sections.items():
            ...
            keywords = {
                ...
                'EPLI': ['employment', 'practices', 'claim'],
                'LIQUOR': ['liquor', 'liability'],
                'WC': ['workers', 'compensation', 'accident']
            }
...
def save_results(all_results: Dict):
    ...
    output_file = Path("qc_extraction_results.json")
    txt_file = Path("qc_extraction_content.txt")
...
def main():
    ...
    files_to_test = [
        "divine_cna_Package_ocr_output.txt",          # Global Indemnity
        # "taj_berkshire_Package_ocr_output.txt",          # Global Indemnity
        # "tikal_Package_ocr_output.txt",         # Tikal
        # "aaniya_Package_ocr_output.txt",        # Aaniya
        # "jk_Package_ocr_output.txt",            # JK
        # "khsuhi_Package_ocr_output.txt",        # KhSuhi
        # "labwork_Package_ocr_output.txt",       # Labwork
        # "westside_Package_ocr_output.txt",      # Westside
        # "traveler_ocr_output.txt",              # Traveler
        # "package2_ocr_output.txt",              # Package2
        # "policy_ocr_output.txt",                # Policy
    ]
